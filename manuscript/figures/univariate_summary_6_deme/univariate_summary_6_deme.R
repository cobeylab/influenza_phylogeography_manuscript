...
antigenDF = dbGetQuery(comboDb, paste('SELECT',paste(parNames,collapse=','),', sweepID,tropics1TrunkPro, tropics2TrunkPro, tropics1AgLag, tropics2AgLag, extinct, excessDiversity, fluLike FROM pooled_results',sep=' '))
...
for(i in 1:length(parNames)){
	...
	pearsonR = cor.test(c(plotDF[,i],plotDF[,i]),c(plotDF$tropics1TrunkPro, plotDF$tropics2TrunkPro))$estimate
	CI = cor.test(c(plotDF[,i],plotDF[,i]),c(plotDF$tropics1TrunkPro, plotDF$tropics2TrunkPro))$conf.int
	
	trunkProPlot = ggplot(plotDF) + 
		geom_point(size = pointSize, aes_string(x=parName,y="tropics1TrunkPro")) + 
		geom_point(size = pointSize, aes_string(x=parName,y="tropics2TrunkPro"), alpha = 0.4, colour = 'red') +
		xlab(NULL) +
		ylab(NULL) +
		ylim(c(0,1)) + 
		plot_themes +	
		...
		stat_smooth(method="lm",se=FALSE,size=lineSize, aes_string(x=plotDF[,i], y=c('tropics1TrunkPro','tropics2TrunkPro'))) +
...
for(i in 1:length(parNames)){
	...
	pearsonR = cor.test(c(plotDF[,i],plotDF[,i]),c(plotDF$tropics1AgLag,plotDF$tropics2AgLag))$estimate
 	CI = cor.test(c(plotDF[,i],plotDF[,i]),c(plotDF$tropics1AgLag,plotDF$tropics2AgLag))$conf.int

	agLagPlot = ggplot(plotDF) + 
		geom_point(size = pointSize, aes_string(x=parName,y="tropics1AgLag")) + 
		geom_point(size = pointSize, aes_string(x=parName,y="tropics2AgLag"), alpha = 0.4, colour = 'red') +
		xlab(parse(text=paste(headers$headerNames[headers$parNames==parName]))) + 
		ylab(NULL) + 
		ylim(c(-0.4,0.4)) + 
		plot_themes +	
		theme(axis.text.y=element_blank()) +
		stat_smooth(method="lm",se=FALSE,size=lineSize,  aes_string(x=plotDF[,i], y=c('tropics1AgLag','tropics2AgLag'))) +
...
pdf(paste(plotDir,'univariate_summary_6_deme.pdf',sep='/'),7,5)
grid.draw(figureS1)
