...
csvFiles = ['meanFluxRate','antigenicLagAG1','trunkProportions','meanTMRCA']

PARAMS = ['initialPrR', 'muPhenotype', 'meanStep', 'sdStep', 'relativeN', 'relativeTurnover', 'tropicFractionI0', 'relativeR0', 'seasonalAmplitude', 'betweenDemePro', 'initialTraitA','beta','smithConversion','endYear']
PARAM_TYPES = {
    'initialPrR' : 'REAL',
    'muPhenotype' : 'REAL',
    'meanStep' : 'REAL',
...
if __name__ == "__main__":
    ...
    comboDb.execute(
        "CREATE TABLE pooled_results (runId INTEGER, {0}, extinct INTEGER, excessDiversity INTEGER, fluLike INTEGER, meanFluxRate REAL, antigenicLagAg1 REAL, trunkProportions REAL, meanTMRCA REAL);".format(
            ', '.join([paramName + ' ' + PARAM_TYPES[paramName] for paramName in PARAMS])
        )
    )
