...
def generateJobs(sweepParamDict, steps):
    ...
    initialIs = [totalInitialIs, 0,0,0,0]
    ...
    for paramName in sweepParamDict:
        ...
        for i in np.linspace(sweepParamDict[paramName][0],sweepParamDict[paramName][1],steps):
            ...
            birthRates = [birthRate*relativeTurnover,
                            birthRate,birthRate,
                            birthRate,birthRate]
                            
            deathRates = birthRates
            demeAmplitudes = [0, seasonalAmplitude,seasonalAmplitude,seasonalAmplitude,seasonalAmplitude]
            
            demeBaselines = [relativeR0, 1,1,1,1]
            ...
            initialNs = [int(relativeN*(totalN/(2.0+relativeN))),
                        int(totalN/(2.0 + relativeN)/2), 
                        int(totalN/(2.0 + relativeN)/2), 
                        int(totalN/(2.0 + relativeN)/2), 
                        int(totalN/(2.0 + relativeN)/2)]
                        
            initialIs = [int(totalInitialIs*tropicFractionI0), 
